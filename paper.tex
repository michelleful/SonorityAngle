...
epenthesis is to occur. Let us fix the sonority distance between the two consonants of the cluster at -2, but vary their distance from V: %TODO : check if it should be -2 or +2 
...
In this paper, I proposed two desiderata for a metric of sonority contour distance, and suggested {\sc Sonority Angle} as a concrete metric that fit these criteria. In this section, I examine the weaknesses of {\sc Sonority Angle} as a metric, as well as possible extensions.
...
%TODO : draw diagram showing what happens with the different C1's
\ex. \begin{tikzpicture} [shorten >=1pt,scale=0.33]
                      \draw [<-] (-3,6.5) -- (-3, 0.5) ; % axis
                      \node at (-3, 7.0) {Sonority}; % axis label
					 % left diagram
...
\noindent {\bf Horizontal distance.} In this paper, I assumed that the horizontal distance between C1 and C2, and C1 and V, is the same, and arbitrarily fixed at 1, in calculating {\sc Sonority Angle}. This is an arbitrary choice, and potentially an important one, as when the horizontal distance gets large, changes occur in the cluster hierarchy:

\ex. TODO: stick in what happens if horizontal distance is 100 and the sonority scale remains the same

Since this horizontal distance is along the dimension of time, the assumption that the two distances is the same is also likely incorrect. If we choose a different horizontal distance for C1-C2 versus C1-V, the cluster hierarchy would also be likely to change.
...
\noindent {\bf Why these vectors?} In the sonority contour faithfulness approach, the relevant comparison is between C1-C2 and C1-V. But why these two vectors? In particular, why not C1-C2 versus V-C2?
...
So far, I have employed the {\sc Sonority Angle} metric purely in the analysis of epenthetic processes. However, there are other possible applications.
...
\ex. \a. Syncope allowed when rising sonority cluster is created
     ...
     \z.
     \b. Syncope disallowed when level or falling sonority cluster would be created
     \a. \textipa{pIk@tIN} $\sim$ \textipa{pIktIN} `picketing'
     \b. \textipa{k@p\ae s@ti} $\sim$ \textipa{k@p\ae sti} `capacity'
     \z.
     \citep{hooper.1978}, cited in \citep{flemming.2008}

On the other hand, syncope in Hebrew (and others?) is said to work according to the principles of Syllable Contact:

\ex. TODO: Hebrew etc examples

More work needs to be done to reconcile these two.
