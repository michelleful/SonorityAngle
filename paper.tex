...
In the default idiolect, epenthesis occurs between C2 and C3 to form [CC\textipa{1}C].
...
In this idiolect, all sonority rises and plateaus --- that is, all clusters with a {\sc Sonority Angle} less than 1.5 --- are conflated together. Similarly, all clusters with a {\sc Sonority Angle} greater than 1.5 are conflated together, with the default preference being to epenthesise on the right.

Borrowing a constraint from \citet{rose.2000}, I use {\sc Align-Left} as the markedness constraint that prefers [CC\textipa{1}C] over [C\textipa{1}CC].

\ex. Align-Left: every consonant must be aligned with the left edge of some prosodic word. % TODO: cite

\vspace{-1cm}
\begin{center} \renewcommand*\arraystretch{1.2}
\scalebox{1}[1]{\begin{tabular}[t]{|rrl||c|} \hline 
\multicolumn{3}{|c||}{Input:~/\textipa{j@-kft-o}/} & {\sc Align-Left} \\[0.5ex]
\hline \hline a. & & \textipa{j@kifto} & 2,4!,5 \\
\hline b. & \ding{43} & \textipa{j@kfito} & 2,3,5 \\
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}
...
\hline \hline a. & \ding{43} & \textipa{j@gd1fo} & 1.37 & \\
\hline b. & & \textipa{j@g1dfo} & 0.59 & $\ast$! \\
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}
     \c. Fall-Fall
...
\hline \hline a. & \ding{43} & \textipa{jams1to} & 2.11 $\ast$ & \\
\hline b. & & \textipa{jam1sto} & 2.03 $\ast$ & $\ast$! \\
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}
     \d. \{Rise, Plateau\}-Fall
\begin{center} \renewcommand*\arraystretch{1.2}
\scalebox{1}[1]{\begin{tabular}[t]{|rrl||c|c|} \hline 
\multicolumn{3}{|c||}{Input:~/\textipa{j@-kft-o}/} & {\sc Ident(SonAngle)}$<$1.5 & {\sc Align-Left} \\[0.5ex]
\hline \hline a. & & \textipa{j@kfito} & 2.11 $\ast$!  & \cellcolor{lightgray} \\
\hline b. & \ding{43} & \textipa{j@kifto} & 0.59 & \cellcolor{lightgray}$\ast$ \\
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}


\bigskip
     
In a second idiolect discussed in Section 5.2.2 of \citep{rose.2000}, clusters behave much as in the default, except that when C2 is [r] and C3 is sonorant, epenthesis occurs on the right instead.
...
\ex. \a. Behaviour of clusters with C2=/r/ and C3=sonorant under idiolect 1:
\begin{center} \renewcommand*\arraystretch{1.2}
\scalebox{1}[1]{\begin{tabular}[t]{|rrl||c|c|} \hline 
...
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}		 
     \b. Behaviour of clusters with C2=/r/ and C3=sonorant under idiolect 2:
\begin{center} \renewcommand*\arraystretch{1.2}
\scalebox{1}[1]{\begin{tabular}[t]{|rrl||c|c|} \hline 
...
In the third idiolect, discussed in Section 5.2 of \citep{rose.2000}, most clusters epenthesise as [C\textipa{1}CC]. This includes clusters 

\ex. TODO

In other words, the default preference is to epenthesise as [C\textipa{1}CC], but this is overridden when C2-C3 constitutes a sonority rise or plateau, and C1-C2 constitutes a sonority fall. 
...
\ex. Anchor-L-Root: An element at the left edge of the root corresponds to an element at the left edge of a heavy syllable. \citep[(45)]{rose.2000}

Armed with this constraint, we can cast this behaviour as the following ranking in {\sc Sonority Angle} terms:

\ex. {\sc Ident(SonAngle)}$<$1.5 $\gg$ {\sc Anchor-L-Root}
...
These two datapoints can be explained by retaining the constraint ranking of idiolect 3, but moving the dividing line 
...
Complete tables illustrating the behaviour of the four idiolects are given in Appendix A. % TODO
...
\ex. \a. Idiolect 1: (sonority rises and plateaus) vs (sonority falls)
     \b. Idiolect 2: (sonority rises and plateaus and RN) vs (other sonority falls)
     \c. Idiolect 3: (sonority rises and plateaus) vs (sonority falls)
     \d. Idiolect 4: (sonority rises) vs (sonority plateaus and falls)

% TODO: diagram showing the various cut-off points.

The {\sc Sonority Rise} metric can achieve the same linear separation for idiolects 1, 3 and 4, but notice that this is not possible for idiolect 2, because % TODO
...
Variable placcement of these two constraints with respect to {\sc Align-Left} and {\sc Anchor-Left Root}, which themselves can be variably ranked, predict three of the four idiolects.

\ex. Rose's constraint rankings for idiolects 1, 3 and 4.
    \a. Idiolect 1: {\sc SyllCon} $\gg$ {\sc Dep} $\gg$ {\sc Align-L} $\gg$ {\sc Anchor-L Root}
        \begin{itemize}
          \item [CC\textipa{1}C] unless C1-C2 violates {\sc SyllCon}
                (is a sonority rise or plateau) and C2-C3 would not.
        \end{itemize}
     \b. Idiolect 3: {\sc SyllCon} $\gg$ {\sc Dep} $\gg$ {\sc Anchor-L Root} $\gg$ {\sc Align-L} 
        \begin{itemize}
          \item [C\textipa{1}CC] unless C2-C3 violates {\sc SyllCon}
                (is a sonority rise or plateau) and C1-C2 would not.
        \end{itemize}
     \c. Idiolect 4: {\sc SyllCon(Loose)} $\gg$ {\sc Dep} $\gg$ {\sc Anchor-L Root} $\gg$ {\sc Align-L} $\gg$ {\sc SyllCon}
        \begin{itemize}
          \item [C\textipa{1}CC] unless C2-C3 violates {\sc SyllCon(Loose)} 
                (is a sonority rise) and C1-C2 would not.
        \end{itemize}

Thus far, the logic has been very similar to the {\sc Sonority Angle} analysis. Explaining the special behaviour of RN clusters in idiolect 2, requires invoking the {\sc *R-Son} constraint used in the analysis of idiolect ?? of coda clusters.

\ex. Rose's constraint ranking for idiolect 2
     \a. {\sc *R-Son} $\gg$ {\sc SyllCon} $\gg$ {\sc Dep} $\gg$ {\sc Align-L} $\gg$ {\sc Anchor-L Root}
% TODO: sample tableau?

In addition, the {\sc *MedialLight} constraint from Rose's analysis of the medial two-consonant clusters must be invoked to prevent double epenthesis forming [C\textipa{1}C\textipa{1}C] in the case where both C1-C2 and C2-C3 constitute violations of {\sc SyllCon}.
% comment on need for *CCC?

Overall, therefore, the {\sc Sonority Angle}-based analysis is slightly more economical than the Syllable Contact-based analysis by Rose, by not requiring any additional machinery to account for the behaviour of RN clusters in idiolect 2.

\section{CCCC clusters}

In this last part of our case study of Chaha, we turn to the behaviour of quadriconsonantal clusters. These epenthesise as [CC\textipa{1}CC] or [C\textipa{1}CC\textipa{1}C]. 
I will show that the rankings already obtained for CCC clusters predict almost exactly which of the CCCC clusters should display variation and in which direction.

\subsection{Predictions of CCC rankings}

The undominated nature of the constraint *CCC, which we used to trigger epenthesis among the triconsonantal clusters, also triggers epenthesis in quadriconsonantal clusters. Notice that using this as the markedness constraint predicts exactly the two outcomes that are directly observed.

Given the general form of the ranking {\sc Ident(SonAngle)}$<n$ $\gg$ {\sc Align-Left},
the expected behaviour of four-consonant clusters is as follows.



% Show that with the rankings we obtained for the various idiolects in the previous section, we predict all and only the clusters observed among 4Cs.
