...
\ex. \label{sonangle_formula} Formula: {\sc Son$\measuredangle$} = $arctan(V-C_1) - arctan(C_2-C_1)$
...
\ex. \a. {\sc Son$\measuredangle$}(NT) =  $arctan(6-3) - arctan(1-3)$ = 2.35
     \b. {\sc Son$\measuredangle$}(TT) =  $arctan(6-1) - arctan(1-1)$ = 1.37
     \c. {\sc Son$\measuredangle$}(TN) =  $arctan(6-1) - arctan(3-1)$ = 0.27
...
\multicolumn{3}{|c||}{Input:~/TF/} & {\sc Id(S$\measuredangle$)}$<$2.0 & {\sc Id(S$\measuredangle$)}$<$1.5 & {\sc *CompCoda} & {\sc Id(S$\measuredangle$)}$<$1.0 & {\sc Id(S$\measuredangle$)}$<$0.5 \\[0.5ex]
...
\multicolumn{3}{|c||}{Input:~/FT/} & {\sc Id(S$\measuredangle$)}$<$2.5 & {\sc Id(S$\measuredangle$)}$<$1.5 & {\sc *CompCoda} & {\sc Id(S$\measuredangle$)}$<$1.0 & {\sc Id(S$\measuredangle$)}$<$0.5 \\[0.5ex]
...
\ex. {\sc Ident(Son$\measuredangle$)}$<$n $\gg$ $\mathcal{M}$
     \a. Scenario 1: {\sc Son$\measuredangle$}(C\textsubscript{1}C\textsubscript{2}), {\sc Son$\measuredangle$}(C\textsubscript{2},C\textsubscript{3})$<$n \\
...
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}
\vspace{0.5em}
      \b. Scenario 2: {\sc Son$\measuredangle$}(C\textsubscript{1}C\textsubscript{2})$>$n, {\sc Son$\measuredangle$}(C\textsubscript{2},C\textsubscript{3})$<$n \\
...
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}
\vspace{0.5em} \newpage
     \c. Scenario 3: {\sc Son$\measuredangle$}(C\textsubscript{1}C\textsubscript{2})$<$n, {\sc Son$\measuredangle$}(C\textsubscript{2},C\textsubscript{3})$>$n \\
...
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}
\vspace{0.5em}
     \d. Scenario 4: {\sc Son$\measuredangle$}(C\textsubscript{1}C\textsubscript{2}), {\sc Son$\measuredangle$}(C\textsubscript{2}C\textsubscript{3}) $>$ n \\ \label{relevantscenario}
...
\ex. Scenario: {\sc Son$\measuredangle$}(C\textsubscript{2}C\textsubscript{3}) $>$ {\sc Son$\measuredangle$}(C\textsubscript{1}C\textsubscript{2}) $>$ n \\ \label{tableauabove}
...
\hline \hline a. & &  C\textsubscript{1}C\textsubscript{2}\textipa{@}C\textsubscript{3} &  2.0 $\ast$!  & \cellcolor{lightgray} 2.0 $\ast$ & \cellcolor{lightgray} \\
\hline b.  &\ding{43}  & C\textsubscript{1}\textipa{@}C\textsubscript{2}C\textsubscript{3} &  1.7  & \cellcolor{lightgray} 1.7 $\ast$ & \cellcolor{lightgray} $\ast$ \\
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}

As the tableau in \ref{tableauabove} illustrates, under a universal ranking of {\sc Ident(Son$\measuredangle$)}$<$n constraints, [C\textsubscript{1}\textipa{@}C\textsubscript{2}C\textsubscript{3}] would be the winning candidate. 

Under free-ranking where {\sc Ident(Son$\measuredangle$)}$<$1.5 is the least stringent constraint to outrank markedness, we have the opposite result.

\ex. Opposite result under free ranking where all more stringent {\sc Ident(Son$\measuredangle$)}$<$n constraints are ranked below $\mathcal{M}$.

\vspace{-2em}
\begin{center} \renewcommand*\arraystretch{1.2}
\scalebox{1}[1]{\begin{tabular}[t]{|rrl||c|c|c|} \hline 
\multicolumn{3}{|c||}{Input:~/C\textsubscript{1}C\textsubscript{2}C\textsubscript{3}/} &  {\sc Ident(Son$\measuredangle$)}$<$1.5 &$ \mathcal{M}$ & {\sc Ident(Son$\measuredangle$)}$<$1.9 \\[0.5ex]
\hline \hline a. & \ding{43} &  C\textsubscript{1}C\textsubscript{2}\textipa{@}C\textsubscript{3} &  2.0 $\ast$ & & \cellcolor{lightgray} 2.0 $\ast$ \\
\hline b. &  & C\textsubscript{1}\textipa{@}C\textsubscript{2}C\textsubscript{3} &  1.7 $\ast$ &  $\ast$! & \cellcolor{lightgray}  1.7 $\ast$ \\
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}

If both epenthesis sites would result in a {\sc Sonority Angle} larger than the least stringent $n$ to outrank markedness, the candidate that satisfies markedness, [C\textsubscript{1}C\textsubscript{2}\textipa{@}C\textsubscript{3}], wins instead. There would be conflation of all clusters with {\sc Sonority Angles} above and below $n$.
...
However, out of the three clusters FT, NF and RN with a sonority distance of -1, RN has the largest {\sc Sonority Rise}, and therefore is expected to be more resistant to epenthesis than NF and FT. This is contrary to criterion \ref{desideratum_b}. If RN undergoes epenthesis, therefore, we predict FT and NF to do the same by the {\sc Sonority Rise} metric.
...
\ex. \a. More sonorous plateaus are more likely to undergo epenthesis under the {\sc Sonority Angle} metric, whereas all the level clusters should have equal likelihood of being broken up by epenthesis under {\sc Sonority Rise}.
...
Epenthesis does not break up word-initial sonorant-C clusters.  This is likely due to the fact that
...
\ex. {\sc Sonority Rises}  of sonorant-initial clusters using revised sonority scale
 ...
 \begin{center}
  \begin{tikzpicture}[shorten >=1pt,->,scale=0.5]
    ...
    \draw (axisstart) -- (axisend);
   \node (xaxislabel) at (2.6 * 15,-2.0) {\textsc{SonRise}};
...
\ex. Table showing epenthesis patterns for clusters of varying sonority patterns:\footnote{No specific examples are available in \citep{rose.2000} for the gaps in the table, but their pattern of epenthesis is implied in the text and Rose's analysis.}
...
%	\node (GF) at (2.0344439358*8, 1.5) {GF};
  ...
  \node (xaxislabel) at (2.7*8,-0.5) {{\sc Son$\measuredangle$}};
  % dividing line
...
\begin{longtable}{ccccc}
	& Underlying form      & Surface form & {\sc Son$\measuredangle$}(C\textsubscript{1},C\textsubscript{2}) & {\sc Son$\measuredangle$}(C\textsubscript{2},C\textsubscript{3}) \\ \hline
...
\scalebox{1}[1]{\begin{tabular}[t]{|rrl||c|c|} \hline 
\multicolumn{3}{|c||}{Input:~/\textipa{j@-krm-o}/} & {\sc Ident(Son$\measuredangle$)}$<$1.9 & {\sc Align-Left} \\[0.5ex]
\hline \hline c. & \ding{43} & \textipa{j@kr1mo} & 1.89 & \\
\hline d. & & \textipa{j@k1rmo} & 0.12 & $\ast$! \\
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}
...
\hline \hline a. & \frownie & \textipa{j@rm1do} & 2.36 $\ast$! & \cellcolor{lightgray} \\
\hline b. & \ding{43} & \textipa{j@r1mdo} & 1.89 & \cellcolor{lightgray}$\ast$ \\
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}
...
\hline \hline a. & \ding{43} & \textipa{j@rm1do} & & \cellcolor{lightgray}2.36 $\ast$ & \cellcolor{lightgray} \\
\hline b. & & \textipa{j@r1mdo} & $\ast$! & \cellcolor{lightgray}1.89 & \cellcolor{lightgray}$\ast$ \\
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}
...
\hline b. & & \textipa{n1.\textnormal{C}1\textnormal{C.C}1.n@} & 2,4!,5,7 & \\
\hline \end{tabular}} \renewcommand*\arraystretch{1} \end{center}
...
Given the general form of the ranking {\sc Ident(Son$\measuredangle$)}$<$n $\gg$ {\sc Align-Left} $\gg$ {\sc Anchor-Left Root}, the expected behaviour of four-consonant clusters is as follows: all /CCCC/ clusters surface as [CC\textipa{1}CC], except when {\sc Son$\measuredangle$}(C\textsubscript{2},C\textsubscript{3}) exceeds n and {\sc Son$\measuredangle$}(C\textsubscript{1},C\textsubscript{2}) and {\sc Son$\measuredangle$}(C\textsubscript{3},C\textsubscript{4}) are both smaller than n. Illustrative tableaux are shown below for n=1.5:
...
Given the general form of the ranking {\sc Ident(Son$\measuredangle$)}$<$n $\gg$ {\sc Anchor-Left Root} $\gg$ {\sc Align-Left}, the default behaviour is to epenthesise as [C\textipa{1}CC\textipa{1}C]. This preference is overridden when at least one of {\sc Son$\measuredangle$}(C\textsubscript{1},C\textsubscript{2}) and {\sc Son$\measuredangle$}(C\textsubscript{3},C\textsubscript{4}) are greater than n and {\sc Son$\measuredangle$}(C\textsubscript{2},C\textsubscript{3}) is less than n.
...
\begin{tabular}{|l|ccc|c|cccc|} \hline
         & \multicolumn{3}{c|}{{\sc Son$\measuredangle$s}} & Observed & \multicolumn{4}{c|}{{\sc Predictions of idiolect}} \\
...
\noindent {\bf Changes in C\textsubscript{1}.} In the discussion of Chaha triconsonantal clusters in  $\S$\ref{CCCanalysis}, I glossed over how the metric should be computed if C\textsubscript{1} changes from the input to the output.
This was the case in clusters such as /rfk/, which emerge as [nf\textipa{1}k] due to morphophonological processes. Ultimately, this was not an issue because the behaviour of all such clusters was independently determined by {\sc Jussive-R}, but this raises the question of how {\sc Sonority Angle} should be computed if C\textsubscript{1} changes. Currently, it is undefined, as it is unclear how to measure the angle between underlying /C\textsubscript{1}C\textsubscript{2}/=/RF/ and surface [C\textsubscript{1}V]=[NV] (shown below on the left). A similar situation arises if underlying /C\textsubscript{1}C\textsubscript{2}/=/NF/ and surface [C\textsubscript{1}V]=[FV] (shown on the right):
...
Several solutions are possible: retain the sonority of the underlying C\textsubscript{1} for that of the output value; use the sonority of the output C\textsubscript{1} as the underlying value; or take the angle formed by the meeting of the two contours, wherever that occurs. 

The two case studies examined above present insufficient data to conclude if any of these interpretations is correct. It is worth noting, however, that alternative approaches such as {\sc Sonority Rise} do not face this issue, as computing the gradients of the underlying contour /C\textsubscript{1}C\textsubscript{2}/ and the surface contour [C\textsubscript{1}V] is trivial even when the underlying and surface C\textsubscript{1}'s differ.
...
\begin{tikzpicture}[scale=0.8,shorten >=1pt,->]
  ...
  \draw (axisstart) -- (axisend);
  \node  (0point) at (0.0,0) {};
  \node (xaxislabel) at (0.06*300,-0.8) {{\sc Son$\measuredangle$}};
\end{tikzpicture}
...
So far, I have employed the {\sc Sonority Angle} metric solely in the analysis of sonority-driven epenthesis. However, there are other possible applications.
...
{\sc Sonority Angle} was offered here as a concrete instance of the class of metrics that conform to the two desiderata. In $\S$\ref{issues}, I discussed several issues regarding the assumptions that need to be incorporated into its formulation. More perceptual experiments and cross-linguistic investigation of sonority-driven epenthesis needs to be done to pin down the specifics of the metric. In addition, I discussed three other sonority-based phenomena that may shed additional light on the true nature of sonority contour faithfulness.
